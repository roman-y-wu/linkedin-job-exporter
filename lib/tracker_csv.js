(function (globalScope) {
  ...
  const LEGACY_CSV_HEADERS = ['job_key', ...CSV_HEADERS];

  const STATUS_VALUES = ['Saved', 'Applied', 'Interview', 'Rejected', 'Ghosted'];

  function isExactHeaderMatch(headers, expectedHeaders) {
    if (!Array.isArray(headers) || !Array.isArray(expectedHeaders)) return false;
  ...
  function isHeaderMatch(headers) {
    return isExactHeaderMatch(headers, CSV_HEADERS) || isExactHeaderMatch(headers, LEGACY_CSV_HEADERS);
  }
  ...
  function parseCsv(text) {
    ...
    const activeHeaders = headers.length > 0 ? headers : CSV_HEADERS;
    const objects = dataRows.map((dataRow) => {
      const byHeader = {};
      for (let i = 0; i < activeHeaders.length; i += 1) {
        byHeader[activeHeaders[i]] = dataRow[i] == null ? '' : String(dataRow[i]);
      }

      const item = {};
      for (const header of CSV_HEADERS) {
        item[header] = byHeader[header] == null ? '' : String(byHeader[header]);
      }
      return item;
    });
